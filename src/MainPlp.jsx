...
import { StyleSheet, Text, View, FlatList, TouchableOpacity, TouchableHighlight ,Image, ScrollView, SafeAreaView, Modal, Alert, TouchableWithoutFeedback,ActivityIndicator } from 'react-native'
...
useEffect(()=>{
  ...
  if (products) {
    products.forEach((item) => {
      if (
        ...
        item.category.parentCategory.parentCategory.name===targetParentCategory
      ) {
  ...
  if (products) {
    products.forEach((item) => {
      if (item.category && item.category.name === targetCategory && item.category.parentCategory.parentCategory.name===targetParentCategory &&
         item.sizes) {
  ...
  useEffect(() => {
    ...
    setFilteredDataArray(
      products.filter((product) => 
        ...
        product.category.parentCategory.parentCategory.name===targetParentCategory   
       )
    );
  ...
  if (products) {
    products.forEach((item) => {
      if (item.category.name === targetCategory && item.category.parentCategory.parentCategory.name===targetParentCategory) {
  ...
  const navigateToMainPDP = (itemCategory,itemId) => {
    ...
    if(currentPage && currentPage.length>0 && currentPage[currentPage.length-1]==='mainPDP'){
      popFromStack(navigation);
    }
    if(currentPage && currentPage.length>0 && currentPage[currentPage.length-1]!=='mainPDP'){
      pushToStack('mainPDP');
 ...
 {/* <Text>{currentPageIndex} {currentPageIndexCategory} {targetParentCategory} </Text> */}
    {/* <Text>{currentPage}</Text> */}
      <View style={{padding:'4%',}}>
          <TouchableOpacity onPress={() => { popFromStack(navigation)}} style={{ }}>
            <View style={{flexDirection: 'row',alignItems: 'center'}}>
            <Image source={back}/>
            <Text style={{ paddingLeft: 10, color: 'black', textAlign: 'center',fontFamily:'sans-serif' }}>{targetCategory.toUpperCase()}{tul}</Text>
            </View>
            <Text style={{marginLeft:'7%',fontSize:10,color:'#949292'}}>{filteredDataArray.length} Items {seeMoreFilterCatefory} </Text>
          </TouchableOpacity>
      </View>
      {/* {
          ...
          <TouchableWithoutFeedback onPress={handleSortModalClose}>
            <View style={styles.modalContainer}>
              <View style={styles.modalContent1}>
                ...
                <View style={{ width: '100%', borderBottomWidth: 1, borderColor: '#dbd9d9' }}/>
...
<View style={{ flexDirection: 'row', alignItems: 'center' }}>
  <View
    style={{
      width: 25,
      height: 25,
      backgroundColor: color === 'white' ? '#FFFFFF' : colorMap[color] || '#FFFFFF',
      borderRadius: color === 'white' ? 12 : 12,
      borderWidth: color === 'white' ? 1 : 0, // Add border only if color is white
      borderColor: color === 'white' ? 'black' : 'transparent', // Border color red if color is white
      marginBottom: '4%',
      marginLeft: '5%'
    }}
  />
      ...
      >
        ...
        <Text style={{ marginLeft: 8, color: '#00338D', fontWeight: '300' }}>{size}</Text>
              ...
              {priceRanges.map((range, index) => (
               ...
               />
                <Text style={{ marginLeft: 8, color: '#00338D', fontWeight: '300' }}>
                {range.max === 99999 ? `Rs. ${range.min} - Above` : range.max === 499 ? `Rs. ${range.max} and Below` : `Rs. ${range.min}-${range.max}`}
                </Text>
               </View>
              ))}
           </View>
        ...
        <>
           <View style={styles.container}> 
              ...
              <TouchableOpacity
               ...
               <Image source={{ uri: item.imageUrl[0] }} onPress={() => navigation.navigate('AboutScreen')} style={{ width: 180, height: 250, borderRadius: 10,}} />
               </TouchableOpacity> 
               <TouchableOpacity onPress={()=>{
                                               handleLovePress(item.id)
                                               lovedItems.includes(item.id)?forNavigate('WishList'):''
                                              }} style={{ position: 'absolute', top: 10, right: 10 }}>
                <Image  source={lovedItems.includes(item.id)?love1:love2}  style={{ height: 25, width: 25 }} />
               </TouchableOpacity>
              </View>
